...
assert.match(campaignPackSource, /CAMPAIGN_ABILITY_PACK_VERSION = "2026-09-17-draft-13"/, "The exact-wording card refresh must use a new import revision");
...
assert.match(campaignPackSource, /forged from the six crystals sacrificed to cast it/, "Emerald Crown must use its exact six-crystal wording");
...
assert.match(campaignPackSource, /Should the crown be struck four times, it will shatter/, "Emerald Crown must use the exact four-strike wording");
assert.match(campaignPackSource, /cast a spell that is level 6 or below/, "Emerald Claim must only contest spells through Level 6");
assert.match(campaignPackSource, /taking 5d8 \+ spellcasting ability modifier emerald damage/, "Emerald Judgment must retain its exact damage wording");
assert.match(campaignPackSource, /resistance to water type Elemental or Devotional damage/, "Sea Blood Constitution must use the exact damage wording");
assert.match(campaignPackSource, /Taking damage while being within water and at 0 HP negates this effect/, "Brackish Hold must include its exact damage exception");
assert.match(campaignPackSource, /Crab Carapace:<\/strong> Your skin hardens akin to a crab's shell\. You gain \+2 AC but lose 10 feet of movement/, "Seashape must use the exact Crab Carapace wording");
...
assert.match(campaignPackSource, /All creatures of your choice within 10 ft cone must make a Dexterity save or take 2d8 slashing damage or be restrained in coral for one round/, "Coral Waves must retain the exact damage-or-restraint wording");
assert.match(campaignPackSource, /Mountain’s Reach and Weight:[\s\S]*changed from 30 feet to 5 feet/, "Monkey must include its exact variable-length staff tradeoff");
assert.match(campaignPackSource, /You gain temporary hit points equal to your twice proficiency bonus at the start of your turn/, "Goat must use its exact recurring temporary HP rule");
...
assert.match(campaignPackSource, /Level 6: The bell will alert you even during your sleep, provided it is equipped/, "Dog must use the exact sleep-alert progression");
...
assert.match(campaignPackSource, /name: "Astral Aegis of Cancer"[\s\S]*?For every spell level above 1, you may choose to target an additional target[\s\S]*?rating: 0,[\s\S]*?status: "Clarify"/, "Cancer must use the exact upcast wording and campaign-balanced review rating");
...
assert.match(campaignPackSource, /name: "Radiant Veil of Virgo"[\s\S]*?end one stack of a negative condition[\s\S]*?\(spell level\)d3 hit points[\s\S]*?rating: 0,[\s\S]*?status: "Clarify"/, "Virgo must use the exact condition-stack and d3 healing wording");
...
assert.match(campaignPackSource, /name: "Twilight's Decree"[\s\S]*?actionCost: "1 Full Action"[\s\S]*?magicStage: "4"[\s\S]*?a Constitution Saving Throw or an Arcana Saving Throw/, "Twilight's Decree must use its revised Full Action, Stage 4, and exact concealment save");
...
assert.match(cardManagerSource, /westernRulesRefreshIds[\s\S]*campaign-lotus-stellar-tides-aquarius[\s\S]*campaign-lotus-astral-aegis-cancer[\s\S]*campaign-lotus-radiant-veil-virgo/, "Re-importing must refresh the revised Aquarius, Cancer, and Virgo rules and descriptions");
...
assert.match(cardManagerSource, /exactWordingRefreshIds[\s\S]*campaign-unassigned-stage-of-pride/, "Re-importing must refresh the full exact-wording card set");
assert.match(cardManagerSource, /if \(shouldRefreshDescription\) \{[\s\S]*nextCustomFields\[CARD_DESCRIPTION_KEY\] = seed\.card\.customFields\[CARD_DESCRIPTION_KEY\]/, "Re-importing must refresh the visible card description alongside the exact rules text");
