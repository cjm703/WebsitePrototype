...
const dmCardManager = read("src/app/components/dm-card-manager-section.tsx");
...
const playerTheme = read("src/app/components/player-theme.tsx");
const dmCustomizeSection = read("src/app/components/dm-customize-section.tsx");
const dmInterfaceStickerManager = read("src/app/components/dm-interface-sticker-manager.tsx");
const interfaceNoteSticker = read("src/app/components/interface-note-sticker.tsx");
...
parse(dmCardManager, { sourceType: "module", plugins: ["typescript", "jsx"] });
...
parse(playerTheme, { sourceType: "module", plugins: ["typescript", "jsx"] });
parse(dmCustomizeSection, { sourceType: "module", plugins: ["typescript", "jsx"] });
parse(dmInterfaceStickerManager, { sourceType: "module", plugins: ["typescript", "jsx"] });
parse(interfaceNoteSticker, { sourceType: "module", plugins: ["typescript", "jsx"] });
...
assert.match(edge.slice(edge.indexOf("async function resolveSessionPlayerId"), edge.indexOf("function requireDM")), /profile\.data\?\.loginLocked === true[\s\S]*Session revoked/);
assert.match(routeBody("/auth-codes/profiles", "/auth-codes/:profileId"), /profileOrder:[\s\S]*\.sort\(/);
...
assert.match(loginPage, /typeof profile\?\.loginLocked !== "boolean"/);
assert.match(loginPage, /PROFILE REGISTRY UNAVAILABLE — PLAYER LOGIN DISABLED/);
assert.match(loginPage, /profileOrder[\s\S]*\.sort\(/);
assert.match(dmArea, /Lock this profile from login/);
assert.match(dmArea, /movePlayerProfile\(player\.id, -1\)/);
assert.match(dmArea, /movePlayerProfile\(player\.id, 1\)/);
assert.match(dmArea, /next\.map\(\(player, profileOrder\)/);
...
assert.match(dmArea, /onPersistTags=\{\(next\) => persistTags\("card", next\)\}/);
assert.match(dmArea, /function normalizePlayerForEditor\(player: PlayerData\): PlayerData/);
assert.match(dmArea, /const handleEditPlayer = \(player: PlayerData\) =>/);
assert.match(dmArea, /scrollIntoView\(\{ behavior: "smooth", block: "start" \}\)/);
assert.match(dmArea, /onClick=\{\(\) => handleEditPlayer\(player\)\}/);
assert.match(dmItemManager, /"Create Tag"/);
assert.match(dmItemManager, /Two-Handed Weapon/);
assert.match(dmItemManager, /Add Weapon Details/);
assert.match(dmItemManager, /Displayed Damage/);
assert.match(dmItemManager, /Damage Dice/);
assert.match(dmItemManager, /Roll Formula/);
assert.match(dmItemManager, /Add \$\{field\.damageAttribute \|\| "STR"\} modifier/);
assert.match(dmCardManager, /type CardWorkspaceStage = "basics" \| "rules" \| "access"/);
assert.match(dmCardManager, /const renderBasicsStage = \(\) =>/);
assert.match(dmCardManager, /const renderUnifiedRulesStage = \(\) =>/);
assert.match(dmCardManager, /const renderAccessStage = \(\) =>/);
assert.match(dmCardManager, /xl:sticky xl:top-4/);
assert.match(dmCardManager, /showAdvancedProfile \? "Hide Advanced" : "Advanced Profile"/);
assert.match(dmCardManager, /showRulesAdvanced \? "Hide Advanced" : "Advanced"/);
assert.match(dmCardManager, /quickRollSlots\.length \? `Rolls \(\$\{quickRollSlots\.length\}\)` : "Add Roll"/);
assert.match(dmCardManager, /hasBuiltInCardTracker\(editingCard\) \? "Tracker" : "Add Tracker"/);
assert.match(dmCardManager, /cardSectionBlocks\.length \? `Sections \(\$\{cardSectionBlocks\.length\}\)` : "Add Section"/);
assert.match(dmCardManager, /FINAL CHECKLIST/);
assert.match(dmCardManager, /const createCardTag = async \(\) =>/);
assert.match(dmCardManager, /await onPersistTags\(\[\.\.\.cardTags, nextTag\]\)/);
assert.match(dmCardManager, /customFields: \{ \.\.\.editingCard\.customFields \}/);
assert.doesNotMatch(dmCardManager, /stripInactiveTagCustomFields/);
assert.doesNotMatch(dmCardManager, /setEditorPanel|setMechanicsView|\{true &&/);
assert.doesNotMatch(dmItemManager, />Weapon Damage Roll</);
...
assert.match(itemCombatRules, /ITEM_INFO_WEAPON_DAMAGE_KEY = "Weapon Damage"/);
assert.match(itemCombatRules, /getWeaponDamageDisplay/);
...
assert.match(personalFiles, /with \$\{attributeLabel\} modifier/);
...
assert.match(itemCombatRules, /usesWeaponDamageAttributeModifier/);
assert.match(personalFiles, /min-w-\[92px\]/);
...
assert.doesNotMatch(loanPage, /RefreshCw|Refresh loan account/);
...
assert.match(edge, /CREDIT_LOAN_OFFER_REFRESH_MS/);
assert.match(edge, /offersRefreshedAt/);
...
assert.doesNotMatch(nexusNomad, /id: "map" as const, label: "Business Map"/);
assert.doesNotMatch(nexusNomad, /activeTab === "map"/);
assert.match(nexusNomad, /businessMap: normalizeOfficeBusinessMap\(raw\.businessMap\)/);
assert.match(facilityMapPage, /<OfficeBusinessMap/);
...
assert.match(facilityMapPage, /countInstalledFacilityAdditions/);
...
assert.match(facilityMapPage, /onPlayerAction=\{handlePlayerAction\}/);
...
assert.match(officeStateApi, /office-state-updates-v2/);
assert.match(officeStateApi, /officeSignalRegistry\.listeners\.add\(onSignal\)/);
assert.match(officeStateApi, /officeSignalRegistry\.listeners\.delete\(onSignal\)/);
assert.match(officeStateApi, /if \(officeSignalRegistry\.channel\) return officeSignalRegistry\.channel/);
assert.equal((officeStateApi.match(/\.subscribe\(\)/g) || []).length, 1);
assert.doesNotMatch(officeStateApi, /removeSupabaseChannelSafely/);
...
assert.match(playerTheme, /createInterfaceNoteSticker/);
assert.match(playerTheme, /kind\?: "image" \| "note"/);
assert.match(dmInterfaceStickerManager, /savePlayerPlacedStickers/);
assert.match(dmInterfaceStickerManager, /normalizePlacedStickers/);
assert.match(dmCustomizeSection, /DMInterfaceStickerManager players=\{players\}/);
assert.match(dmArea, /DMCustomizeSection statusTags=\{statusTags\} players=\{players\}/);
assert.match(intelliInterface, /isInterfaceNoteSticker\(ps\)/);
assert.match(intelliInterface, /<InterfaceNoteSticker key=\{ps\.id\} sticker=\{ps\} \/>/);
assert.match(interfaceNoteSticker, /width: 86 \* scale/);
assert.match(intelliInterface, /loadPlayerPlacedStickers/);
assert.match(intelliInterface, /12_000/);
