...
assert.match(edge, /table === "app_players"[\s\S]+ensureCreditAccountRow\(row\.id\)/);
assert.match(edge, /Could not provision account during login/);
...
assert.match(creditsApi, /CREDITS_SERVICE_UNAVAILABLE/);
...
assert.match(accountPage, /credit-change-rise/);
assert.match(accountPage, /credit-change-fall/);
assert.match(accountPage, /prefers-reduced-motion/);
...
assert.doesNotMatch(nexusNomad, /Unified Credits accounts|Commerce, Workshop, facilities, and manual entries share one audited balance per player/);
assert.match(nexusNomad, /if \(!isDM\)[\s\S]+setBusinessMapPlayers/);
...
assert.match(commerce, /effect\?: string/);
assert.match(commerce, /value=\{draftItem\.effect \|\| ""\}/);
assert.match(commerce, /<RenderFormattedText text=\{item\.effect \|\| ""\}/);
...
assert.match(commerce, /Return to Item Creator/);
assert.match(commerce, /View Store/);
assert.match(commerce, /itemCreatorMinimized/);
assert.doesNotMatch(commerce, /This uses the same item authoring workflow as the DM Area and links the saved item to this product/);
