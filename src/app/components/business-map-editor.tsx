...
import {
  ...
  defaultFacilityAdditionStaffing,
...
import { FACILITY_ADDITION_STAT_KEYS, FACILITY_STAT_META } from "@/lib/facility-depth-model";
...
export function OfficeBusinessMap({
  ...
  const addAddition = () => {
    ...
    const additionCategory = selectedSlot?.acceptedAdditionCategories[0] || "Unassigned";
    const addition = {
      ...createFacilityAddition(additions.length),
      category: selectedSlot?.acceptedCategories[0] || "Unassigned",
      additionCategory,
      ...defaultFacilityAdditionStaffing(additionCategory),
    ...
    <section className="border-t border-[#1A1A2B] pt-2" aria-label="Facility Addition storage">
      ...
      {selected && (
        <div className="mt-2 grid grid-cols-1 gap-2 border border-[#1A1A2B] bg-[#050508] p-2 lg:grid-cols-[110px_minmax(0,1fr)_210px]">
          ...
          <div className="space-y-2">
            {isDM ? <><Field label="Addition Category"><select value={selected.additionCategory} onChange={(event) => { const additionCategory = event.target.value as FacilityAdditionCategory; onUpdate(selected.id, { additionCategory, ...defaultFacilityAdditionStaffing(additionCategory) }); }} className="w-full border bg-transparent px-2 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }}>{FACILITY_ADDITION_CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select></Field><div className="grid grid-cols-3 gap-2"><Field label="Quantity"><input type="number" min="0" max="999" value={selected.quantity} onChange={(event) => onUpdate(selected.id, { quantity: Math.max(0, Number(event.target.value) || 0) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field><Field label="Width"><input type="number" min="1" max="32" value={selected.width} onChange={(event) => onUpdate(selected.id, { width: Math.max(1, Number(event.target.value) || 1) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field><Field label="Height"><input type="number" min="1" max="24" value={selected.height} onChange={(event) => onUpdate(selected.id, { height: Math.max(1, Number(event.target.value) || 1) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field></div><div className="grid grid-cols-2 gap-2"><Field label="Purchase Cost"><input type="number" min="0" value={selected.cost} onChange={(event) => onUpdate(selected.id, { cost: Math.max(0, Number(event.target.value) || 0) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field><Field label="Monthly Upkeep"><input type="number" min="0" value={selected.monthlyUpkeep} onChange={(event) => onUpdate(selected.id, { monthlyUpkeep: Math.max(0, Number(event.target.value) || 0) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field></div><div className="grid grid-cols-2 gap-2"><Field label="Staff Required"><input type="number" min="0" value={selected.staffRequired} onChange={(event) => onUpdate(selected.id, { staffRequired: Math.max(0, Math.round(Number(event.target.value) || 0)) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field><Field label="Staff Provided"><input type="number" min="0" value={selected.staffProvided} onChange={(event) => onUpdate(selected.id, { staffProvided: Math.max(0, Math.round(Number(event.target.value) || 0)) })} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field></div><div className="grid grid-cols-2 gap-1">{FACILITY_ADDITION_STAT_KEYS.map((stat) => <Field key={stat} label={FACILITY_STAT_META[stat].label}><input type="number" value={selected.statModifiers.find((modifier) => modifier.stat === stat)?.amount || 0} onChange={(event) => { const amount = Number(event.target.value) || 0; onUpdate(selected.id, { statModifiers: [...selected.statModifiers.filter((modifier) => modifier.stat !== stat), ...(amount ? [{ stat, amount }] : [])] }); }} className="w-full border bg-transparent px-1 py-1.5 text-[9px]" style={{ color: "#E5ECFF", borderColor: CONTROL_BORDER }} /></Field>)}</div><label className={`${retro.button} flex cursor-pointer items-center justify-center gap-1 py-2 text-[8px]`} style={S_TEXT}><Upload size={9} /> Thumbnail<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) onUploadThumbnail(selected, file); event.currentTarget.value = ""; }} /></label>{uploadProgress != null && <ProgressBar value={uploadProgress} />}<button type="button" onClick={() => onDelete(selected)} disabled={(usage[selected.id] || 0) > 0} className={`${retro.button} flex w-full items-center justify-center gap-1 py-2 text-[8px] disabled:opacity-30`} style={S_RED}><Trash2 size={9} /> Delete</button></> : <><div className="text-[9px]" style={S_DIM}>{Math.max(0, selected.quantity - (usage[selected.id] || 0))} available of {selected.quantity}</div><div className="text-[8px]" style={S_DIM}>{selected.additionCategory} · {selected.cost.toLocaleString()} CR · {selected.monthlyUpkeep.toLocaleString()} CR upkeep</div><div className="text-[8px]" style={S_DIM}>{selected.staffProvided} staff provided · {selected.staffRequired} required</div><div className="space-y-1">{selected.statModifiers.map((modifier) => <div key={modifier.stat} className="flex justify-between text-[8px]"><span style={S_DIM}>{FACILITY_STAT_META[modifier.stat].label}</span><span style={modifier.amount >= 0 ? S_GREEN : S_RED}>{modifier.amount >= 0 ? "+" : ""}{modifier.amount}</span></div>)}</div></>}
