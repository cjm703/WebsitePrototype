...
async function loadPartyColorStateWithPromptFallback(): Promise<PartyColorStateDoc> {
  const [partyState, customizeState] = await Promise.all([
    appStore.loadPartyColorState<PartyColorStateDoc>({ canvasData: "", prompt: "" }),
    appStore.loadDmCustomizeState<DmCustomizePromptState>({}).catch(() => ({})),
  ]);
  const fallbackPrompt = typeof customizeState.partyColorPrompt === "string" ? customizeState.partyColorPrompt : "";
  return {
    ...partyState,
    prompt: partyState.prompt || fallbackPrompt,
  };
}
