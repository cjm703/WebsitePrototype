...
export const appStore = {
  ...
  saveNodeTrees: <T>(rows: T[]) => replaceCollection("app_node_trees", rows),
  ...
  savePlayers: <T>(rows: T[]) => replaceCollection("app_players", rows),

  listDeletedPlayers: <T>() => listCollection<T>("app_deleted_players"),
  saveDeletedPlayers: <T>(rows: T[]) =>
    replaceCollection("app_deleted_players", rows),

  listItems: <T>() => listCollection<T>("app_items"),
  saveItems: <T>(rows: T[]) => replaceCollection("app_items", rows),

  listCards: <T>() => listCollection<T>("app_cards"),
  saveCards: <T>(rows: T[]) => replaceCollection("app_cards", rows),

  listInfos: <T>() => listCollection<T>("app_infos"),
  saveInfos: <T>(rows: T[]) => replaceCollection("app_infos", rows),

  listInfoSubTabs: <T>() => listCollection<T>("app_info_subtabs"),
  saveInfoSubTabs: <T>(rows: T[]) => replaceCollection("app_info_subtabs", rows),

  listNotifications: <T>() => listCollection<T>("app_notifications"),
  saveNotifications: <T>(rows: T[]) =>
    replaceCollection("app_notifications", rows),

  listNews: <T>() => listCollection<T>("app_news"),
  saveNews: <T>(rows: T[]) => replaceCollection("app_news", rows),

  listSites: <T>() => listCollection<T>("app_sites"),
  saveSites: <T>(rows: T[]) => replaceCollection("app_sites", rows),

  listCustomPanelStyles: <T>() => listCollection<T>("app_custom_panel_styles"),
  saveCustomPanelStyles: <T>(rows: T[]) =>
    replaceCollection("app_custom_panel_styles", rows),

  listCustomReactions: <T>() => listCollection<T>("community_custom_reactions"),
  saveCustomReactions: <T>(rows: T[]) =>
    replaceCollection("community_custom_reactions", rows),

  listTags: <T>(kind: "item" | "card" | "info" | "status" | "wiki") =>
    listTagCollection<T>(kind),
  saveTags: <T>(
    kind: "item" | "card" | "info" | "status" | "wiki",
    rows: T[],
  ) => replaceTagCollection(kind, rows),

  loadPlayerLevelCategories: <T>(playerId: string, fallback: T) =>
    loadPlayerDoc("player_level_categories", playerId, fallback),
  savePlayerLevelCategories: <T>(playerId: string, data: T) =>
    savePlayerDoc("player_level_categories", playerId, data),
};
